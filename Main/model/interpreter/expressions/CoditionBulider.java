...
public class CoditionBulider {
    public static double calcc(String expression){
        ...
        if(i>0||j>0) {
            if((j<i&&j!=-1)||i==-1)
                i=j;
            num1 = new Number(MathExpressionPredicate.calc(deli.substring(0, i)));
        ...
        return MathExpressionPredicate.calc(expression);
    }
    ...
    public static double calc(String expression){
        ...
        while(expression.contains("&")||expression.contains("|")){
            ...
            if((j<i&&j!=-1)||i==-1)
                i=j;
            token=MathExpressionPredicate.calc(expression.substring(0, i))+"";
        ...
        token=MathExpressionPredicate.calc(expression)+"";
        queue.addFirst(token);
        while (!stack.isEmpty())
            queue.addFirst(stack.pop());
